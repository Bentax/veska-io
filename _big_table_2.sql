with pre_load as (
    ...
    FROM (
        SELECT
            ...
            any(if(price != 0, price, NULL)) as price,
            min(if(min_price != 0, min_price, NULL)) as min_price,
            max(if(max_price != 0, max_price, NULL)) as max_price
    ...
    select 
        ...
        median(rate * 100) over (partition by market, exchange order by toDate(datetime) asc range between 12*30 preceding and current row) as mean_ref_12,
        ...
        median(rate  * 100) over (partition by market, exchange order by toDate(datetime) asc range between 3*30 preceding and current row) as mean_ref_3,
        ...
        median(rate * 100) over (partition by market, exchange order by toDate(datetime) asc range between 6*30 preceding and current row) as mean_ref_6
    from funding_stream
    where price = 0 and  market in (${markets}) and exchange = '${exchange}' 
),
max_date as (
    select 
        max(datetime) as m_date, 
        if(date_trunc('hour',$__toTime) > m_date, 
            m_date,
            toStartOfInterval(date_trunc('hour',$__toTime), interval if('${exchange}'='dydx',1,8) hour)
        ) as to_filtDate,
        max(if(price!=0, datetime,null)) as max_price_dt_1,
...
stats2 as (
    select  market as m2, max(price) as now_price
    from funding_stream
    where price != 0 and exchange = '${exchange}' and datetime = (select max_price_dt from max_date)
    group by market
)
SELECT 
